...
def _draft_executive_summary(numerics: dict, chunks: list, available: list[str]) -> str:
    ...
    return (
        ...
        f"{s003}. Critically, the CRAT carries **no net estate-tax penalty** from early death — "
        f"if the grantor dies during the {crat_term}-year term, the §2036 estate inclusion is "
        f"offset by the §2055 charitable deduction. For a term-of-years CRAT, if the trust "
        f"instrument names a successor beneficiary, annuity payments may continue to that "
        f"person or the estate for the remaining term {s003}. The "
...
def _draft_planning_objectives(numerics: dict, chunks: list, available: list[str]) -> str:
    ...
    return (
        ...
        f"outcomes\n"
        f"- Any recommended structure must be implementable under the applicable 2015 federal tax law without reliance "
...
def _draft_grat_analysis(numerics: dict, chunks: list, available: list[str]) -> str:
    ...
    return (
        ...
        f"than the CRAT in the prevailing low-rate environment of the 2015 regime {s007}.\n\n"
...
def _draft_crat_analysis(numerics: dict, chunks: list, available: list[str]) -> str:
    ...
    return (
        ...
        f"**Early death during a term-of-years CRAT:** If the income beneficiary (the grantor) dies "
        f"during the {crat_term}-year term, the outcome depends on the trust document. If a "
        f"**successor beneficiary** is named, the annuity payments may continue to that person "
        f"(or to the estate) for the remainder of the fixed term {s003}. If no successor is named, "
        f"the annuity ceases and the remaining corpus passes to the designated charity at that "
        f"time {s003}. In either case, the estate receives an offsetting charitable deduction under "
        f"IRC §2055 for the present value of the charitable remainder, so there is **no net "
        f"estate-tax penalty** from early death — unlike the GRAT, where death causes full corpus "
        f"inclusion with no offset {s003} {s006}. However, the grantor (or the estate) forfeits "
        f"any annuity payments remaining after death if no successor beneficiary is in place.\n\n"
        ...
        f"{s003}.\n\n"
        f"**Distinguishing the projected remainder from the charitable deduction:** The model "
        f"projects that at the end of the {crat_term}-year term, the trust corpus will be worth "
        f"{_n(numerics, '{{crat_remainder_to_charity_usd}}')} (the **projected nominal "
        f"future remainder** to charity, assuming a {_n(numerics, '{{grat_growth_rate}}')} return "
        f"that exactly offsets the {_n(numerics, '{{crat_payout_rate}}')} annual payout). This "
        f"is a **different figure** from the charitable deduction of "
        f"{_n(numerics, '{{crat_charitable_deduction_usd}}')}, which represents the **present "
        f"value** of the charitable remainder interest discounted at the §7520 rate of "
        f"{_n(numerics, '{{section_7520_rate_bps}}')} and calculated using a conservative 4.00% "
        f"growth assumption {s007} {s009}. The reader should note:\n\n"
        f"- {_n(numerics, '{{crat_remainder_to_charity_usd}}')} = projected dollar amount the "
        f"charity is expected to receive at year {crat_term} (nominal future value)\n"
        f"- {_n(numerics, '{{crat_charitable_deduction_usd}}')} = present value of the charitable "
        f"remainder interest (used for the upfront income-tax deduction and estate-tax reduction)\n"
        f"- {_n(numerics, '{{crat_total_annuity_paid_usd}}')} = total annuity payments the client "
        f"receives over {crat_term} years (this is income to the client, not the charitable "
        f"component)\n\n"
        ...
        f"### Estate Tax Treatment — §2036 Inclusion and §2055 Charitable Deduction\n\n"
        f"The CRAT's estate tax treatment differs fundamentally from the GRAT and involves **two "
        f"distinct statutory provisions**. First, the grantor's retained annuity interest in the "
        f"CRAT is an IRC §2036 retained interest, meaning it is included in the gross estate at "
        f"death {s006}. Second, IRC §2055 provides a **charitable estate-tax deduction** for the "
        f"present value of the remainder interest passing to charity, which offsets the §2036 "
        f"inclusion and produces the net estate tax reduction {s010}. It is critical to distinguish "
        f"these provisions: §2036 causes inclusion; §2055 provides the deduction. The deduction "
        f"is NOT authorised by §2036.\n\n"
        f"This is a critical distinction from the GRAT: when a GRAT grantor dies during the term, "
        f"the full corpus is included in the estate with **no** offsetting deduction {s006}. When "
        f"a CRAT grantor dies, the §2036 inclusion is offset by the §2055 charitable deduction, "
        f"resulting in net estate tax savings.\n\n"
        ...
        f"- **No net estate-tax penalty from early death** — unlike the GRAT, the grantor's death "
        f"during the term does not cause uncompensated corpus inclusion in the estate (the §2055 "
        f"charitable deduction offsets the §2036 inclusion). However, for a term-of-years CRAT, "
        f"the annuity may continue to a successor beneficiary or the estate if the trust document "
        f"so provides {s003}\n"
...
def _draft_comparative_analysis(numerics: dict, chunks: list, available: list[str]) -> str:
    ...
    return (
        ...
        f"| Mortality Risk | **High** — full estate inclusion if grantor dies during term {s006} | **Low** — no net estate-tax penalty; annuity may continue to successor or cease, remainder to charity |\n"
        ...
        f"the {crat_term}-year term, the annuity payments to the noncharitable beneficiary may "
        f"**cease** (or continue to a successor beneficiary or the estate if the trust instrument so "
        f"provides) and the remaining trust corpus passes to the designated charitable "
        f"remainder beneficiary {s003}. The estate receives a charitable deduction under IRC §2055 "
        f"that offsets the IRC §2036 inclusion, so there is no net estate tax penalty from early "
        f"death. However, the grantor forfeits any remaining annuity payments (unless a successor "
        f"is named). The CRAT's estate tax benefit is preserved "
        f"regardless of when the grantor dies.\n\n"
        f"### Estate Tax Comparison — Step-by-Step\n\n"
        f"To understand how each strategy produces its stated estate-tax savings, we walk through "
        f"the calculation {s010}:\n\n"
        f"**Baseline (no planning):**\n"
        f"- Gross estate: {_n(numerics, '{{taxable_estate_before_usd}}')}\n"
        f"- Combined marital exemption (2015): $10,860,000.00\n"
        f"- Taxable excess: {_n(numerics, '{{taxable_estate_before_usd}}')} − $10,860,000.00 = "
        f"$5,140,000.00\n"
        f"- Estate tax at {_n(numerics, '{{estate_tax_rate}}')}: $5,140,000.00 × 40% = "
        f"$2,056,000.00 potential liability\n\n"
        f"**After GRAT:** The GRAT removes {_n(numerics, '{{grat_projected_remainder_usd}}')} "
        f"from the taxable estate (the projected remainder passing to children) {s001}. The "
        f"reduced estate is {_n(numerics, '{{taxable_estate_after_grat_usd}}')} "
        f"({_n(numerics, '{{taxable_estate_before_usd}}')} − "
        f"{_n(numerics, '{{grat_projected_remainder_usd}}')}). The taxable excess above the "
        f"married exemption shrinks accordingly, saving "
        f"{_n(numerics, '{{estate_tax_saved_by_grat_usd}}')} at the "
        f"{_n(numerics, '{{estate_tax_rate}}')} rate.\n\n"
        f"**After CRAT:** The CRAT's estate reduction is the charitable deduction of "
        f"{_n(numerics, '{{crat_charitable_deduction_usd}}')} (the present value of the "
        f"charitable remainder under §2055), NOT the full {_n(numerics, '{{crat_trust_corpus_usd}}')} "
        f"corpus — because §2036 causes the retained annuity interest to remain in the estate "
        f"{s006}. The reduced estate is {_n(numerics, '{{taxable_estate_after_crat_usd}}')} "
        f"({_n(numerics, '{{taxable_estate_before_usd}}')} − "
        f"{_n(numerics, '{{crat_charitable_deduction_usd}}')}), saving "
        f"{_n(numerics, '{{estate_tax_saved_by_crat_usd}}')} at the "
        f"{_n(numerics, '{{estate_tax_rate}}')} rate {s010}.\n\n"
        ...
        f"reduces the taxable estate by a larger amount than the GRAT remainder "
        ...
        f"| Estate tax savings | {_n(numerics, '{{estate_tax_saved_by_grat_usd}}')} | {_n(numerics, '{{estate_tax_saved_by_crat_usd}}')} |\n"
        f"| Mortality risk | ~15–20% probability of total loss | No net estate-tax penalty; annuity may continue to successor if named |\n"
        ...
        f"- The recommendation assumes the 2015 tax law remains in effect; material changes to "
...
def _draft_recommendation(numerics: dict, chunks: list, available: list[str]) -> str:
    ...
    return (
        ...
        f"- The recommendation assumes the 2015 tax law in effect at the time of the liquidity event; any material changes to "
...
def _draft_risks_considerations(numerics: dict, chunks: list, available: list[str]) -> str:
    ...
    return (
        ...
        f"this risk — the estate receives a charitable deduction under §2055 offsetting the §2036 "
        f"inclusion regardless of when the grantor dies {s003}.\n\n"
...
def _draft_citations_disclosures(numerics: dict, chunks: list, available: list[str]) -> str:
    ...
    return (
        ...
        f"8. **IRC §2033 / §2036 / §2055** — Estate inclusion and deduction rules: §2033 governs "
        f"GRAT inclusion on grantor death during term (full corpus, no offset); §2036 governs CRAT "
        f"retained interest inclusion; §2055 provides the charitable estate-tax deduction that "
        f"offsets §2036 inclusion {s006}\n"
